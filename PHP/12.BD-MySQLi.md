...
// Introducimos la query que modificará los caracteres a utf-8
...
Otro ejemplo, siguiendo la documentación oficial, es el siguiente:
...
$usuario = 'root';
$pass = '';
// Definimos el array dentro de la variable global $_SESSION que contendrá el listado de errores producidos
...
try{
	...
	echo "Conexión no disponible", $e->getMessage(), "\n";
	// Almacenamos el error en la variable $_SESSION
	array_push($_SESSION["error"], "Conexión no disponible");
	exit;
}
try{
	// Establecemos la conexión
	$resultado = $conexion->query(
		"SELECT sku, nombre, introDescripcion, descripcion, img, precio, precioOferta, moneda 
		FROM ofertas ORDER BY sku");
} catch (Exception $e){
	echo "Consulta fallida", $e->getMessage(), "\n";
	// Almacenamos el error en la variable $_SESSION
	array_push($_SESSION["error"], "Consulta fallida");
	exit;
}
$ofertas=$resultado->fetchAll(PDO::FETCH_ASSOC);
// $ofertas mostrará un array asociativo, ver http://php.net/manual/es/pdostatement.fetch.php para más opciones PDO
?>
```
**[Regresar al índice](#indice)**
